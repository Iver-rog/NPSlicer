...
pub fn main(blender:&mut crate::Blender) {
    ...
    let first_layer_infill = first_layer_perimeters.clone().into_iter()
        .flat_map(|polygon| polygon.offset(-((nr_of_perimeters as f32) + 0.5)*layer_w))
        .flat_map(|polygon| generate_infill_allong_x_2d(polygon, layer_h) );
    ...
    let first_layer_paths = first_layer_perimeters.into_iter()
            ...
            .flat_map(|(i,polygon)| {
                let offset = (( (brim as isize)-(i as isize) )as f32)*layer_w;
...
fn infill_test(){
    ...
    let paths = generate_infill_allong_x(bounds, &mesh);
    for path in paths {
        blender.path(&path);
    }
    // blender.show();
...
fn generate_infill_allong_y(bounds:Polygon,mesh:&MeshCollider) -> Vec<Path> {
    ...
    for (i,collumn) in xz_vals.iter().enumerate() {
        print!("x{i}|");
        for (x,z) in collumn{
            match z {
                Some(z) => print!(" x:{x:.1} z:{z:.1} |"),
                None => print!(" x:{x:.1} z:None|"),
            }
        }
        println!("");
    }
...
fn generate_infill_allong_x_2d(bounds:Polygon,height:f32) -> Vec<Path> {
    let aabb = &bounds.outer_loop().aabb;
    let (min,max) = ( aabb.x_min.floor() as isize, aabb.x_max.ceil() as isize);
